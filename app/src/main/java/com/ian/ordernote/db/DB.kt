...
//            var whereClause = "${DBConfig().CO_MOBILE}=? and ${DBConfig().CO_NAME}=?"
    ...
    fun setOrder(order: OrderInfo): Long {
        ...
        try {
            ...
            values.put(DBConfig().CO_CONTENT_OTHER, order.content_other)
    ...
    fun getOrder(): ArrayList<OrderInfo>? {
        ...
        if(cursor?.count!! > 0) {
            while (cursor?.moveToNext()) {
                ...
                order.content = cursor.getInt(cursor.getColumnIndex(DBConfig().CO_CONTENT))
                order.content_other = cursor.getString(cursor.getColumnIndex(DBConfig().CO_CONTENT_OTHER))
